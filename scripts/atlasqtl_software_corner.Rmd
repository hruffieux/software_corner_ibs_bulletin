...
even galaxies. As well as growing in dimension, the collected datasets are also 
...
to be a hotspot and is assigned a horseshoe prior --- the corresponding global-local 
specification adapts to the overall problem sparsity (thanks to the global scale 
$\sigma_0$), while flexibly capturing the individual hotspot effects (thanks to the Cauchy 
tail of the local scale $\lambda_s$). A graphical representation of the model is 
provided in Figure \@ref(fig:figgm) and its full specification 
<!-- and inference algorithm are  -->
is given in @ruffieux2020global.
...
inference, with 
<!-- implements a variational inference algorithm based on  -->
a structured mean-field factorisation and efficient batch updates. The algorithm 
is augmented with a simulated annealing scheme that improves the exploration of 
highly multimodal spaces by introducing so-called "temperature" parameters 
controlling the degree of separation of the modes for a series of "heated" 
distributions (@ruffieux2020global). 
...
setting with Gaussian errors. Hereafter we discuss a use case in the context of 
expression quantitative trait locus (eQTL) analysis, in which 
\emph{hotspot genetic variants}, controlling many gene expression traits at once, 
may be responsible for important functional mechanisms underlying specific 
disease endpoints --- such studies aim to clarify the genetic architecture of 
diseases by estimating associations between several thousand responses (molecular traits, 
such as genomic, proteomic, metabolomic levels) and up to a few million candidate 
predictors (genetic variants).
...
The material (incl. Rmarkdown document) used in the example below is available at 
...
our in-house R package `echoseq`; these data emulate real genotypes 
(single nucleotide polymorphisms, "SNPs") located in the *LYZ* gene region. The 
gene expression traits consist of monocyte levels before and after immune 
...
propensity circumvents \emph{ad-hoc} choices of top-level variance parameters (for which
...
which can be accessed by running `?atlasqtl` in the console. 
...
Bayesian FDR control can also be used (@ruffieux2017efficient and @ruffieux2020global).
...
on the probability of candidate predictors to be involved in associations (@ruffieux2021epispot). 
Specifically, the effects of a (possibly high-dimensional) predictor-level 
vector of covariates are estimated using a secondary spike-and-slab regression 
at the top of the model hierarchy. The R package is available at 
