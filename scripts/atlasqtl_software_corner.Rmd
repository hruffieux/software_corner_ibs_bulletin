...
```{r figgm, echo=FALSE, fig.cap="Graphical representation of the model with $n$ samples, $p$ candidate predictors and $q$ responses. The shaded nodes are observed, the others are inferred.", fig.align = 'center', out.width="70%"}
...
chromosome 12. The gene expression traits consist of B-cell levels. 

### Case study: an eQTL analysis to understand gene regulation in B cells
...
permutation-based Bayesian FDR threshold is employed, where the "null-case" 
...
large-response settings, see @ruffieux2020global.
...
inverse temperature, with initial temperature of $2$ and grid size of $10$
schedule --- these choices can be altered via the argument `anneal`. The 
...
additional settings (e.g., seed for random number generator, tolerance for the 
variational algorithm, maximum number of iterations, checkpointing, etc.) can be 
chosen as detailed on the help page of the function, which can be accessed by 
running `?atlasqtl` in the console. 
...
```
\normalsize 
A serial execution of the above analysis on a standard 
laptop\footnote{$2.8$GHz Quad-Core Intel Core i7 machine} takes less than
$10$ seconds. 

The object returned by `atlasqtl` contains posterior quantities 
...
A summary for these quantities can optionally be based on a supplied Bayesian 
FDR threshold (here $5\%$):
...
\normalsize 
Alternatively a $p \times q$ matrix of FDR estimates can be 
...
The Manhattan-type plot indicates the position of the hotspots after Bayesian 
FDR control: 
\footnotesize
```{r manhattan-plot, include = TRUE, fig.align = 'center', fig.cap="Hotspot sizes, as obtained using the S3 function plot.atlasqtl. The crosses indicate the simulated sizes of the four hotspots.", fig.align = 'center', out.width="90%"}
sim_hs <- rowSums(pat) # simulated hotspot sizes (4 hotspots)
                       # pat is a p x q binary 'pattern matrix' 
                       # locating the simulated associations
plot(obj_atlasqtl, thres = thres_fdr, fdr_adjust = TRUE, 
     ylim_max = max(sim_hs))
points(which(sim_hs>0), sim_hs[sim_hs>0], pch = 4)
```
\normalsize 
Thanks to the scalability of the variational implementation, a 
permutation-based Bayesian FDR control can also be used (@ruffieux2017efficient 
...
`atlasqtl` hierarchical framework to encode and leverage information on the 
